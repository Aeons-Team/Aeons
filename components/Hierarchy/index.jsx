...
function HierarchyItem({ item, depth }) {
  ...
  const { activateContextMenu, currentFile, setCurrentFile, currentFileAncestors } = useAppContext();
    ...
    <div>
...

      {
        ...
        <div
          ...
          {
            item.children.filter(child => child.type == 'file').map(child => (
              <HierarchyItem key={child.id} item={child} depth={depth + 1} />
            ))
          }
        </div>
...
export default function Hierarchy() {
  ...
  const root = fileSystem.hierarchy.getFile('root')
